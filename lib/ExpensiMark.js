...
export default class ExpensiMark {
    constructor() {
        ...
        this.rules = [
             ...
             *
             * Note: currently we are allowing mentions in a format of @+19728974297, @+19728974297@expensify.sms and @username@example.com
             * The username can contain any combination of alphanumeric letters, numbers, and underscores
             */
            {
                name: 'userMentions',
                regex: new RegExp(`[\`.a-zA-Z]?@+(${CONST.REG_EXP.EMAIL_PART}|${CONST.SMS.E164_REGEX})(?!((?:(?!<a).)+)?<\\/a>|[^<]*(<\\/pre>|<\\/code>))`, 'gm'),
                replacement: (match) => {
                    if (!Str.isValidMention(match)) {
                        return match;
                    }
